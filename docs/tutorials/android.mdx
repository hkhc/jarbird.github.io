...
com/hkhc/jarbird-samples/android).
...
```
# Kotlin
```kotlin title="build.gradle.kts" {4,8}
// after android {}
...
```
# Kotlin
```kotlin title="build.gradle.kts" {6}
android.libraryVariants.configureEach {
    jarbird {
        pub(name) {
            mavenLocal()
